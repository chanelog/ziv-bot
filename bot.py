...
def add_user_vps(vps_name, username, password, duration, created_by='bot'):
...
async def manage_users(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(text, parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup([
            ...
            [btn("🔄 Perpanjang","renew_user_menu")],
            back_btn()
        ]))
...
async def create_user_password(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📅 Ketik *durasi* (hari), contoh: `30`", parse_mode=ParseMode.MARKDOWN)
...
async def create_user_duration(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ok, result = add_user_vps(vps_name, username, password, dur, created_by)
...
def main():
    ...
    create_user_conv = ConversationHandler(
        ...
        states={
            ...
            ST_RESELLER_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, create_user_duration)],
        },
